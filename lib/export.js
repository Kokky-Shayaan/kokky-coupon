...
// A PDF sized exactly to the coupon image in pixels (single page).
...
// A PDF sized to an exact PHYSICAL size in centimetres (single page). Use this
// so the coupon prints at the real-world size the user picked, regardless of
// the image's pixel resolution.
export function canvasToPdfCm(canvas, widthCm, heightCm) {
...
// Download a single-coupon PDF at an exact centimetre size.
export function downloadPdfCm(canvas, filename, widthCm, heightCm) {
  canvasToPdfCm(canvas, widthCm, heightCm).save(filename);
}
...
export function buildA4SheetPdf(items, cols = 1, rows = 3) {
  ...
  items.forEach((it, i) => {
    ...
    // If a physical coupon size (in mm) is supplied, print at that exact size
    // (capped to the cell). Otherwise fall back to fitting the cell.
    let w, h;
    if (it.widthMm && it.heightMm) {
      const fit = Math.min(1, cellW / it.widthMm, cellH / it.heightMm);
      w = it.widthMm * fit;
      h = it.heightMm * fit;
    } else {
